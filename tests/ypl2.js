...
const proveGoals = (goals, env) => {
  ...
  for (const rule of ruleList) {
    ...
    if (unify(terms[0].args, goal.args, env)) {
      let newGoals = goals.next;
      for (let i = terms.length - 1; i > 0; --i) {
        newGoals = {goal: terms[i], next: newGoals};
      }
      proveGoals(newGoals, env);
    }
    env.pop();
  }
};
...
const solve = (name, args, callback) => {
  ...
  const goals = {goal: {name, args}, next: {goal: env => (callback(env), false), next: null}};
